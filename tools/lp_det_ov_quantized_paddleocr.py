...
def vis_plate(img, boxes, scores, cls_ids, conf=0.5, class_names=None, plate_num=""):
    ...
    for i in range(len(boxes)):
        ...
        txt_bk_color = color = (0, 255, 0)
        cv2.rectangle(
            img,
            (x0, y0 + 1),
            (x0 + txt_size[0] + 200, y0 - int(3.9*txt_size[1])),
            txt_bk_color,
            -1
        )
        ...
        if ret_val:
            ...
            if dets is not None:
                ...
                frame_bbox = vis_plate(frame, final_boxes, final_scores, final_cls_inds,
                                conf=0.7, class_names=COCO_CLASSES, plate_num=plate_num)
